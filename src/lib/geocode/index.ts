...
export async function geocodePlace(place: string): Promise<GeocodeResult> {
  ...
  if (provider === 'opencage') {
    ...
    const warnings: string[] | undefined = tz ? undefined : ['timezone_inferred'];

    const result: GeocodeResult = { latitude: lat, longitude: lon, timezoneSource: tz ? 'provider' : 'inferred' };
    if (address !== undefined) (result as any).resolvedAddress = address;
    if (timezone !== undefined) (result as any).timezone = timezone;
    if (warnings !== undefined) (result as any).warnings = warnings;
    return result;
  }
  ...
  const result: GeocodeResult = { latitude: lat, longitude: lon, timezoneSource: 'inferred' };
  if (address !== undefined) (result as any).resolvedAddress = address;
  if (timezone !== undefined) (result as any).timezone = timezone;
  return result;
}
