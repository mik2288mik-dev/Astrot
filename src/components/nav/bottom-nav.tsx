...
export const BottomNav = memo(function BottomNav() {
  ...
  return (
    <nav
      ...
      className="fixed bottom-0 left-0 right-0 z-50 bg-[rgb(var(--astrot-surface))] border-t border-[color:rgb(var(--astrot-border)/0.08)] shadow-sm"
              ...
              <li key={tab.key} className="flex items-center justify-center">
                <Link href={tab.href} aria-label="Открыть карту" className="relative block">
                  <motion.div
                    ...
                    className="relative w-14 h-14 rounded-full bg-[rgb(var(--astrot-accent))] text-white flex items-center justify-center shadow-md z-10 transition-transform hover:scale-[1.05] active:scale-[0.95]"
          ...
          return (
            <li key={tab.key} className="flex items-center justify-center">
              <Link
                ...
                className="flex h-full w-full flex-col items-center justify-center text-xs transition-transform hover:scale-[1.05] active:scale-[0.95]"
